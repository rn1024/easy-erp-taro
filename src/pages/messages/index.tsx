  ...
  return (
    <View className="messages-page">
      <View className="messages-page__content">
        {/* 标签页 */}
        <View className="messages-page__tabs">
          <View className="messages-page__tabs-container">
            {tabs.map(tab => (
              <View
                key={tab.id}
                className={`messages-page__tab ${
                  selectedTab === tab.id ? 'messages-page__tab--active' : ''
                }`}
                onClick={() => setSelectedTab(tab.id)}
              >
                <Text className="messages-page__tab-text">{tab.name}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View className="messages-page__list">
          {filteredMessages.length === 0 ? (
            <View className="messages-page__empty">
              <Text className="messages-page__empty-text">暂无消息</Text>
            </View>
          ) : (
              ...
              return (
                <View
                  ...
                  className={`messages-page__message-card ${
                    !message.isRead ? 'messages-page__message-card--unread' : ''
                  }`}
